function [Abx,Aby,Bbx,Bby,Bbxm,Bbym,Bbxn2,Bbyn2,Bbpx,Bbpy, ...
          ...
          Rx,Ry,Xn,Yn,Xn2,Yn2,Xm,Ym,Xl,Yl,Xpn,Ypn]=setops(Nx,Ny,dom,bc,nf,df);
...
%   [zypn,wypn]=zwgl(Npy+1);
    ...
    Bbxn2=sparse(diag(wxn2));
    Bbyn2=sparse(diag(wyn2));

    Bbxm=sparse(diag(wxm));
    Bbym=sparse(diag(wym));

    Bbpx=sparse(diag(wxpn));
    Bbpy=sparse(diag(wypn));
